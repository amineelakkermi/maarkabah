...
export default function RentPoliciesSection() {
  ...
  useEffect(() => {
    loadPolicies();
    lookupService.getContext({ sections: ["RentPolicyEnums" as any] })
      .then((response: any) => {
    ...
    <div className="rounded-xl overflow-hidden mk-surface">
      ...
      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)}>
        <div className="flex flex-col justify-between h-full w-full">
          ...
          <DrawerFooter className="mt-4 pt-4 border-t border-mk-border justify-stretch"><Button variant="outline" onClick={() => setDrawerOpen(false)}>{T("Cancel", "إلغاء", ar)}</Button><Button variant="primary" className="flex-1" disabled={!form.nameAr.trim() || saving} onClick={savePolicy}>{saving && <Loader2 size={14} className="animate-spin" />}{editingPolicy ? T("Save changes", "حفظ التعديلات", ar) : T("Create policy", "إنشاء السياسة", ar)}</Button></DrawerFooter>
        </div>
      </Drawer>
    </div>
  );

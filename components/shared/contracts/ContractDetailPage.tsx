    ...
    return (
      <ExtendCardShell ar={ar}>
        <div className="text-center">
          ...
          <Button variant="primary" onClick={() => { setSubmitted(false); setReason(""); }}>
            {T("Done", "تم", ar)}
          </Button>
        </div>
      </ExtendCardShell>
    ...
    <ExtendCardShell ar={ar}>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-1.5">
          {reasons.map((r) => (
            <Chip key={r} size="sm" active={reason === r} onClick={() => setReason(r)}>
              {r}
            </Chip>
          ))}
        </div>
      ...
      <div className="flex items-center justify-between mk-caption mb-2.5 py-1">
        ...
        {editing ? (
          <div className="flex items-center gap-1.5">
            ...
            <IconButton size="sm" variant="active" onClick={saveEdit} aria-label={T("Save", "حفظ", ar)}>
              <Check size={12} strokeWidth={3} />
            </IconButton>
            <IconButton size="sm" variant="ghost" onClick={cancelEdit} aria-label={T("Cancel", "إلغاء", ar)}>
              <X size={12} />
            </IconButton>
          </div>
        ) : (
...
export interface ContractDetailPageProps {
  /** Route to return to the contracts list. */
  contractsListPath: string;
  /** Builds the pickup/handover path for a given contract id. */
  pickupPath: (id: string) => string;
  /** Builds the return processing path for a given contract id. */
  returnPath: (id: string) => string;
  /** Role of the current user; controls whether the discount is editable. */
  role?: "owner" | "frontdesk";
}
  ...
  if (!contract) {
    return (
      <div className="py-24 text-center">
        ...
        <Link href={contractsListPath} className="mk-body-sm text-mk-blue-500 no-underline">
  ...
  return (
    <div dir={ar ? "rtl" : "ltr"}>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <Link
          href={contractsListPath}
          aria-label={T("Back to contracts", "العودة للعقود", ar)}
          className="inline-flex items-center justify-center rounded-full border-0 cursor-pointer transition-colors duration-base ease-standard shrink-0 w-10 h-10 bg-white shadow-[var(--shadow-card)] text-mk-ink-600 hover:text-mk-ink-900 no-underline"
        >
          ...
          <Link
            ...
            className={`inline-flex items-center justify-center gap-2 font-semibold rounded-pill border cursor-pointer transition-[background,color,border-color] duration-base ease-standard active:scale-[0.98] select-none px-5 py-3 mk-body-sm text-white no-underline shadow-[var(--shadow-glow-blue)] ${
              contract.status === "late"
                ...
                : "bg-mk-blue-500 border-transparent hover:bg-mk-blue-600"
            }`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-4 mb-4 items-start">
        ...
        <div className="rounded-[22px] p-5 mk-surface">
          ...
          <Button variant="outline" size="sm" className="w-full mb-4" onClick={() => setShowMap(true)}>
            <MapPin size={13} />
